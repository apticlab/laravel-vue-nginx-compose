<template>
  <div
    class="bg-white h-full fixed left-0 top-0 z-20 shadow-xs transition-all duration-200 ease-in flex flex-col"
    :class="{
      'w-16': is_collapsed,
      'w-56': !is_collapsed
    }"
  >
    <div
      class="h-20 text-center flex flex-col justify-center font-medium text-xl"
    >
      <div
        :class="{
          hidden: is_collapsed
      ...
      >
        <template v-if="show_text">
          <span class="text-indigo-600 font-medium">App</span>Logo
        </template>
      </div>
      <div
      ...
      >
        <p class="m-0 text-indigo-600 leading-relaxed">AL</p>
      </div>
    </div>
    <div class="flex flex-col flex-grow">
        ...
        >
          ...
          >
            <i
              ...
              class="h-6 leading-6 text-lg pl-4 pr-2 text-gray-500"
            ></i>
            <span v-if="show_text">
              {{ item.meta.label }}
            </span>
      ...
      >
        <div class="flex flex-row select-none">
          ...
          >
            <span
              class="fas fa-angle-double-left text-gray-600 rotate-initial-cw"
              :class="is_collapsed ? 'rotate-0-cw' : 'rotate-180-cw'"
            ></span>
            <span class="pl-2 text-gray-600 overflow-hidden" v-if="show_text">
              {{ "sidebar_collapse" | translate }}
            </span>
          </div>
        </div>
      </div>
